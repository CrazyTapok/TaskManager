...
public class TaskControllerTests
{
    ...
    public async Task GetByIdAsync_ReturnsOkResult_WhenTaskExists()
    {
        ...
        var task = _fixture.Build<TaskManager.Core.Models.Task>().With(task => task.Id, taskId).Create();
    ...
    public async Task AddAsync_ReturnsCreatedAtActionResult_WhenTaskIsCreated()
    {
        ...
        var createdTask = _fixture.Build<TaskManager.Core.Models.Task>()
                                  .With(task => task.Title, taskRequest.Title)
                                  .Create();
        _mockTaskService.Setup(service => service.AddAsync(It.IsAny<TaskManager.Core.Models.Task>(), _cancellationToken))
    ...
    public async Task UpdateAsync_ReturnsNoContentResult_WhenTaskIsUpdated()
    {
        ...
        var taskId = Guid.NewGuid();
        var taskRequest = _fixture.Create<TaskRequest>();
        _mockTaskService.Setup(service => service.UpdateAsync(It.IsAny<TaskManager.Core.Models.Task>(), _cancellationToken))
                        .ReturnsAsync(true);
